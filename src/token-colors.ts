import { ThemeMode, ThemeSubtype, TokenColor } from './models'
...
export default function buildTokenColors(
  ...
  const darkFG = p.blueGray[200]
  const darkFG2 = p.blueGray[400]
  const darkHeaders = p.cyan[400]
  const darkKeywords = p.blue[400]
  const darkClasses = p.purple[400]
  const darkInterfaces = p.neutral[500]
  const darkNumbers = p.pink[400]
  const darkStrings = p.teal[400]
  const darkFunctions = p.neutral[200]
  const darkEscape = p.orange[550]
  ...
  const darkTokens = {
    ...
    punctuation: darkFG2,
    // Comments
    comments: p.neutral[570],
    commentTags: p.blueGray[600],
    commentParams: p.teal[600],
    // Numbers
    numbers: darkNumbers,
    // Strings
    strings: darkStrings,
    stringInterpolation: darkKeywords,
    stringEscape: darkEscape,
    // Regex
    regex: darkStrings,
    regexCharacterClass: p.red[200],
    regexGroup: darkFG2,
    regexQuantifier: darkHeaders,
    // Keywords
    keywords: darkKeywords,
    operators: darkFG2,
    operatorsSpecial: darkKeywords,
    storage: darkKeywords,
    controls: darkKeywords,
    imports: darkKeywords,
    // Functions
    functions: darkFunctions,
    // Types
    thisKeyword: darkClasses,
    interfaces: darkInterfaces,
    classes: darkClasses,
    decorators: darkClasses,
    // Variables
    variables: darkFG,
    // HTML/CSS
    tags: darkKeywords,
    tagsClose: darkKeywords,
    attributes: darkClasses,
    attributeIDs: darkEscape,
    cssClasses: darkHeaders,
    cssProperties: darkClasses,
    // Serializable
    keys1: darkHeaders,
    keys2: darkKeywords,
    keys3: p.indigo[400],
  ...
  const tokensBySubtype = {
    soft: darkTokens,
    hard: darkTokens,
  }

  const c = tokensBySubtype[subtype]
